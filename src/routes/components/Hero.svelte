<script>
	import HeroImg from '$lib/assets/bg-hero.webp';
</script>

<header class="my-8 px-8 md:px-24">
	<div class="hero w-full bg-center bg-cover rounded-3xl" style="background-image:url({HeroImg});">
		<div
			class="flex items-center justify-center w-full h-full bg-gray-900 bg-opacity-50 md:py-40 py-24 rounded-3xl"
		>
			<div class="text-center">
				<div class="container px-4 mx-auto">
					<div class="max-w-4xl mx-auto text-center">
						<span class="text-gray-200 font-semibold uppercase tracking-widest">Tours & Travel</span
						>
						<h2 class="mt-8 text-4xl md:text-6xl font-bold text-gray-100">Chindra Wisata</h2>
						<p class="max-w-3xl mx-auto mb-10 text-xl md:text-3xl text-gray-300">
							Creating Affordable Travel Experiences
						</p>
						<a class="btn bg-lime-400" href="/product">Lihat Paket</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</header>
